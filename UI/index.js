...
function clickAction(cellTDelement,row,col,cellNr)
	...
	//code history:
	//appreciating react and vue: this code was built pre react/vue days, and still is pure JS, no react/vue stuff here.
	//the two independent lines of JS toggled the className and the true/false value in the array
	//due to a bug somewhere else in code, the class showed 'clicked' but the array became false.
	//i.e. the two 'states got opposite of each other soemhow (class showed 'clicked', value in array was false)
	//very time the click event fired, both of them toggled, making them both out of sync forever
	//ie until another mystery 'bug' could fix it; the bug needed fixing of course, ..
	//.. but as a matter of robustness,
	//.. later placed the toggling-on of className and arrayvalue  within a single {} block as below 
	//.. (and likewise toggling off)
	
	//If multi click disabled, remove previous click item
	if(lastClickedElement)
		{lastClickedElement.classList.remove('clicked');clickState[cellNr-1] = false;}
